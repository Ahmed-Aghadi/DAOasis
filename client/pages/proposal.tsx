...
import {Anchor, Avatar, Breadcrumbs, Button, Center, Group, Paper, Tabs, Text, Textarea, Title} from "@mantine/core";
        ...
        <Layout>
            ...
            <StyledTabs defaultValue="discussions">
                ...
                <Tabs.Panel value={"discussions"}>
                    <form style={{position: "relative"}}
                          onSubmit={async (e: any) => {
                              ...
                              try {
                                  ...
                                  const creator = safeContext.safeAuthSignInResponse?.eoa
                                  const multiSigId = router.query.id as string
                                  const data = await addReply({
                                      multiSigProposalId: multiSigId,
                                      description: content,
                                      creator: creator!
                                  })
                                  setProposalData(data.response.data)
                    ...
                    {
                        proposalData?.replies.slice(0).reverse().map((reply, index) => (
                    ...
                    <Paper my="md" p='md' bg="#eeebf7" radius="lg">
                        ...
                        {proposalData?.transactionHash && (
                            <ProposalAction createdAt={proposalData.createdAt} creator={proposalData.creator}
                                            description={proposalData.description} id={proposalData.id}
                                            title={proposalData.title} multiSigId={proposalData.multiSigId}
